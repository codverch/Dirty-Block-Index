...
namespace gem5
{
            ...
            if (pkt->fromCache())
            {
                ...
                if (pkt->needsWritable())
                {
                    ...
                    if (blk->isSet(CacheBlk::DirtyBit))
                    {
                        ...
                        DirtyBlockIndex.insert(make_pair(pkt->getAddr(), CacheBlk::DirtyBit));
                    }
                }
    ...
    void
    DBICache::recvTimingSnoopResp(PacketPtr pkt)
    {
        //   DPRINTF(DBICache, "%s for %s\n", __func__, pkt->print());
        ...
        if (!forwardAsSnoop)
        {
            ...
            //    DPRINTF(DBICache, "Got prefetch response from above for addr "
            //                 "%#llx (%s)\n",
    ...
    void
    DBICache::promoteWholeLineWrites(PacketPtr pkt)
    {
            ...
            !pkt->isMaskedWrite())
        {
                    ...
                    //    DPRINTF(DBICache, "packet promoted from Write to WriteLineReq\n");
        }
    }

    void
    DBICache::handleTimingReqHit(PacketPtr pkt, CacheBlk *blk, Tick request_time)
    {
    ...
    void
    DBICache::handleTimingReqMiss(PacketPtr pkt, CacheBlk *blk, Tick forward_time,
                                  Tick request_time)
    {
    ...
    DBICache::recvTimingReq(PacketPtr pkt)
    {
        ...
        if (pkt->cacheResponding())
        {
                    ...
                    //     DPRINTF(DBICache, "Cache above responding to %s: not responding\n",
                    pkt->print());
                               ...
                               bool is_whole_line_write) const
    {
        ...
        if (cpu_pkt->hasSharers() && !needsWritable)
        {
                    ...
                    pkt->setHasSharers();
                    DPRINTF(DBICache, "%s: passing hasSharers from %s to %s\n",
        ...
        pkt->allocate();
        //    DPRINTF(DBICache, "%s: created %s from %s\n", __func__, pkt->print(),
                cpu_pkt->print());
                return pkt;
    }
                                  ...
                                  PacketList &writebacks)
    {
                ...
                //   DPRINTF(DBICache, "%s: Sending an atomic %s\n", __func__,
                bus_pkt->print());
                ...
                //  DPRINTF(DBICache, "%s: Receive response: %s for %s\n", __func__,
    ...
    void
    DBICache::doTimingSupplyResponse(PacketPtr req_pkt, const uint8_t *blk_data,
    ...
    void
    DBICache::recvTimingSnoopReq(PacketPtr pkt)
    {
                // DPRINTF(CacheVerbose, "%s: for %s\n", __func__, pkt->print());
                ...
                {
                           // DPRINTF(DBICache, "Deferring snoop on in-service MSHR to blk %#llx (%s)."
                              "mshrs: %s\n",
                ...
                if (wb_entry)
                {
                              // DPRINTF(DBICache, "Snoop hit in writeback to addr %#llx (%s)\n",
                    ...
                    if (pkt->isEviction())
                    {
                        ...
                        pkt->setBlockCached();
                        // DPRINTF(DBICache, "%s: Squashing %s from lower cache on writequeue "
                                  "hit\n",
                        __func__, pkt->print());
                                  return;
                    }
    ...
    Tick
    DBICache::recvAtomicSnoop(PacketPtr pkt)
    {
                // no need to snoop requests that are not in range.
                if (!inRange(pkt->getAddr()))
                {
    ...
    bool
    DBICache::isCachedAbove(PacketPtr pkt, bool is_timing)
    {
                if (!forwardSnoops)
                    return false;
    ...
    DBICache::sendMSHRQueuePacket(MSHR *mshr)
    {
                ...
                if (tgt_pkt->cmd == MemCmd::HardPFReq && forwardSnoops)
                {
                    ...
                    if (snoop_pkt.cacheResponding())
                    {
                                  ...
                                  // DPRINTF(DBICache, "Upward snoop of prefetch for addr"
                                  " %#x (%s) hit\n",
                    ...
                    {
                                  // DPRINTF(DBICache, "Block present, prefetch squashed by cache.  "
    ...
    void
    DBICache::print(const unordered_map<Addr, bool> &DirtyBlockIndex)
    {
                for (const auto &[key, value] : DirtyBlockIndex)
                {
                    DPRINTF(DBICache, "DirtyBlockIndex: %#x %s\n", key, value ? "true" : "false");
                }
    }
